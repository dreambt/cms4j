...
<div class="main_container container_16 clearfix">
    <form:form modelAttribute="article" id="articleForm" method="post">
            ...
            <table class="display table">
                ...
                <c:forEach items="${articles}" var="article" begin="0" step="1" varStatus="stat">
                    <tr class="gradeA">
                        ...
                        <td><a href="${ctx}/article/audit/${article.id}">
                            <c:choose>
                            <c:when test="${article.status}">【已审核】</a></c:when>
                            <c:otherwise>【未审核】</c:otherwise>
                        </c:choose></a></td>

                        <td><a href="${ctx}/article/allow/${article.id}"><c:choose><c:when
                                test="${article.allowComment}">【允许】</c:when><c:otherwise>【不允许】</c:otherwise></c:choose></a>
                        </td>
                        <td><a href="${ctx}/article/edit/${article.id}">【编辑】</a><a
                                href="${ctx}/article/top/${article.id}"><c:choose><c:when
                                test="${article.top}">【置顶】</c:when><c:otherwise>【未置顶】</c:otherwise></c:choose></a> <a
                                href="${ctx}/article/delete/${article.id}"><c:choose><c:when
        ...
        <button class="button_colour" id="auditAll"><img height="24" width="24" alt="Bended Arrow Right"
                                                         src="${ctx}/static/dashboard/images/icons/small/white/BendedArrowRight.png"/><span>批量审核</span>
        </button>
